...
'''
ndarray 편하게 생성하기 : 
    테스트용으로 데이터를 만들거나 대규모의 데이터를
    일괄적으로 초기화 하는 경우에 사용하는 함수 목록
    arange() : 파이썬의 대표함수인 range와 유사하게 배열생성.
        파이썬의 range와 거의 유사하게 사용할 수 있따.
    ones() : 배열 생성후 모든 요소를 1로 초기화.
        단 인자로 dtype을 지정하지 않으면 float64로 초기화된다.
    zeros() : ones()와 동일 하고 0으로 초기화 된다.
    '''
...
'''
reshape :
    ndarray의 차원의 크기를 변경하는 함수.
    -1을 인수로 지정하면 원본 ndarray와 호환되는
    새로운 shape로 변환해준다.
...
'''
tolist() :
    넘파이 배열을 리스트로 변환하는 함수.
    N차원의 배열을 사용하면 N차원 형태의 리스트가 반환된다.
'''

# 0~7까지의 정수로 구성된 1차원 배열 생성
array2 = np.arange(8)
# 2개의 블록 (깊이)으로 구성된 2행 2열인 3차원 배열로 전환
arrayC1 = array2.reshape((2,2,2))
print('arrayC1:\n', arrayC1)
print('arrayC1: \n', arrayC1.tolist())

# 3차원 ndarray를 2차원 배열로 변환.  X행 1열로 변환하므로 8행1열인 2차원 배열로 변환된다.
