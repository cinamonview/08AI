...
# 오름차순 정렬되었을때 원본배열의 인덱스를 반환 
sort_indices = np.argsort(org_array)
print('타입:', type(sort_indices))
print('결과 인덱스1:', sort_indices)
# 앞에서의 결과를 적용하면 배열이 오름차순으로 정렬된다. 
org_array_asc = org_array[sort_indices]
print('정렬이 적용된 배열:', org_array_asc)
...
만약 점수를 기준으로 정렬하고 싶다면, 점수 배열의 정렬된 인덱스를 argsort
함수로 반환받은 후 이름 배열에 적용하면된다. 
'''